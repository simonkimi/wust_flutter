    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `ClassModel` (`id` INTEGER, `className` TEXT, `teacher` TEXT, `classId` TEXT, `time` TEXT, `classRoom` TEXT, PRIMARY KEY (`id`))');
...
class _$ClassModelDao extends ClassModelDao {
  _$ClassModelDao(this.database, this.changeListener)
      : _queryAdapter = QueryAdapter(database),
        _classModelInsertionAdapter = InsertionAdapter(
            database,
            'ClassModel',
            (ClassModel item) => <String, dynamic>{
                  'id': item.id,
  ...
  static final _classModelMapper = (Map<String, dynamic> row) => ClassModel(
      row['id'] as int,
  ...
  Future<List<ClassModel>> getAll() async {
    return _queryAdapter.queryList('SELECT * FROM ClassModel',
        mapper: _classModelMapper);
  }

  @override
  Future<void> insertModel(ClassModel classModel) async {
    await _classModelInsertionAdapter.insert(
        classModel, OnConflictStrategy.abort);
  }
}
